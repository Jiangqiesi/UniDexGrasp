...
Description: Generate object point clouds with table
"""
...
def sample_projected(_):
    ...
    builder = scene.create_actor_builder()
    builder.add_box_visual(half_size=[1, 1, 0])
    table_actor = builder.build_kinematic()
    ...
    for pose_matrix in pose_matrices:
        ...
        for idx_camera in range(len(camera_eye)):
            ...
            segmentation = camera.get_actor_segmentation()
            ...
            points_opengl_object = position[..., :3][(position[..., 2] != 0) & (segmentation == 1)]
            points_object_world = points_opengl_object @ model_matrix[:3, :3].T + model_matrix[:3, 3]
            pc_object.append(points_object_world)
        
            # OpenGL/Blender: y up and -z forward
            points_opengl_table = position[..., :3][(position[..., 2] != 0) & (segmentation == 2)]
            points_table_world = points_opengl_table @ model_matrix[:3, :3].T + model_matrix[:3, 3]
            pc_table.append(points_table_world[np.linalg.norm(points_table_world, axis=1) < args.table_distance])
            
            

        pc_object = np.concatenate(pc_object)
        pc_object_down_sampled = pc_object[np.random.choice(len(pc_object), args.max_n_points, replace=False)] if len(pc_object) > args.max_n_points else pc_object
        pc_object_sampled = pytorch3d.ops.sample_farthest_points(torch.tensor(pc_object_down_sampled).unsqueeze(0), K=args.num_samples)[0][0]
        pc_object_sampled = (pc_object_sampled - translation) @ rotation_matrix / args.scale
        
        pc_table = np.concatenate(pc_table)
        pc_table_down_sampled = pc_table[np.random.choice(len(pc_table), args.max_n_points_table, replace=False)] if len(pc_table) > args.max_n_points_table else pc_table
        pc_table_sampled = pytorch3d.ops.sample_farthest_points(torch.tensor(pc_table_down_sampled).unsqueeze(0), K=args.num_samples_table)[0][0]
        pc_table_sampled = (pc_table_sampled - translation) @ rotation_matrix / args.scale
        
        pcs.append(np.concatenate([pc_object_sampled, pc_table_sampled]))
